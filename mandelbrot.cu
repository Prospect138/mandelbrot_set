...
__device__ int isInSet(cuda::std::complex<DataType> c, int max_iterations)
{
    cuda::std::complex<DataType> z(0.0f, 0.0f);
    for (int i = 0; i < max_iterations; i++)
    {
        z = cuda::std::pow(z, 2) + c;
        if (cuda::std::norm(z) > normal) 
        {
            return i;
        }   
    }
...
__global__ void calculateScreen(int* d_screen, DataType zoom, DataType offsetX, DataType offsetY, int max_iterations)
{
    ...
    int iters = isInSet(cuda::std::complex<DataType>(point_x, point_y), max_iterations);
